...
from .models import ShopItem, InventoryItem, GoldTransaction, GoldTransactionType
...
class ShopViewSet(viewsets.ReadOnlyModelViewSet):
    ...
    def purchase(self, request, pk=None):
        ...
        if InventoryItem.objects.filter(user=user, item=item).exists():
        ...
        with transaction.atomic():
            ...
            inv_item = InventoryItem.objects.create(user=user, item=item)
            
        return Response(InventoryItemSerializer(inv_item).data, status=status.HTTP_201_CREATED)
